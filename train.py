...
def parse_args():
    ...
    parser.add_argument("-w_i", "--weight_init", type=str, choices=["random", "He", "Xavier", "kaiming", "Xavier_normal", "kaiming_normal"], default="Xavier", help="Weight initialization method")
    ...
    parser.add_argument("-szl", "--hidden_size_list", type=list, default=[256, 256, 128, 64], help="Hidden layer size list")
    ...
    parser.add_argument("-do", "--dropout_p", type=float, required=False, default=0.0)    
...
if __name__ == "__main__":
    ...
    if args.wandb_entity is not None and args.wandb_project is not None:
        ...
        wandb.init(project=f"{args.wandb_project}", 
                            ...
                            name=f"epoch{args.epochs}_batch{args.batch_size}_dropout{args.dropout_p}_optim{args.optimizer}_lr{args.learning_rate}_init{args.weight_init}_layers{args.num_layers}_hidden{args.hidden_size}_activation{args.activation}_weightdecay{args.weight_decay}",
                            config=vars(args))
    else:
        logging = False
