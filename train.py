...
def parse_args():
    ...
    parser.add_argument("-w_d", "--weight_decay", type=float, default=0.005, help="Weight decay (L2 regularization)")
...
if __name__ == "__main__":
    ...
    if args.wandb_entity is not None and args.wandb_project is not None:
        ...
        wandb.init(project=f"{args.wandb_project}", 
                            ...
                            name=f"epoch{args.epochs}_batch{args.batch_size}_optim{args.optimizer}_lr{args.learning_rate}_init{args.weight_init}_layers{args.num_layers}_hidden{args.hidden_size}_activation{args.activation}",
                            config=vars(args))
    else:
        logging = False
